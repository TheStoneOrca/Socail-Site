...
const db = new pg.Client({
  connectionString:
    "postgresql://josephiannuzzelli4561:f5LIBtRr7OQF@ep-billowing-morning-02647692.us-east-2.aws.neon.tech/orcasite?sslmode=require",
  port: 5432,
  database: "orcasite",
  user: "josephiannuzzelli4561",
  password: "f5LIBtRr7OQF",
});
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    if (PasswordCheck) {
      const jwt = Jwt.sign(checkUsername.rows[0], process.env.JWT_KEY, {
        expiresIn: "24h",
      });
      res.json(jwt);
    } else {
...
app.post("/api/getuser", async (req, res) => {
  Jwt.verify(req.body.token, process.env.JWT_KEY, (err, data) => {
    if (err) {
      console.error(err);
...
app.patch("/api/user/:id/edit", async (req, res) => {
  try {
    const userDetails = req.body;
    let checkUser;
    Jwt.verify(req.body.jwtToken, process.env.JWT_KEY, (err, data) => {
      if (err) {
        console.error(err);
        return res.json("Unauthorized");
      }
    ...
    const jwt = Jwt.sign(user.rows[0], process.env.JWT_KEY, {
      expiresIn: "24h",
    });
    res.json(jwt);
  } catch (error) {
